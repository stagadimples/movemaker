...
with order_groups as (
  select 
    ...
    sum(case when orderdate = trunc(sysdate) - 1 then units else 0 end) as sold_rep_units
  from
  (
    select 
    ...
    where
      ...
      trunc(orh.creation_time) between trunc(sysdate) - 31 and trunc(sysdate) - 1
    group by
...
(
  select
    ...
    nvl(o.sold_rep_units, 0) as sold_rep_units,
    case when nvl(o.sold_rep_units, 0) = 0 then 0 else 1 end sale_class,
    row_number() over (
      partition by case when nvl(o.sold_rep_units, 0) > 0 then 1 else 0 end
      order by p.prod_id
    ) as rnk
  from 
...
create table shop_demand_current_features
as
with order_groups as (
  select 
    prod_id,
    sum(case when orderdate between trunc(sysdate) - 7 and trunc(sysdate) - 1 then orders else 0 end) as rep_orders_lst7,
    sum(case when orderdate between trunc(sysdate) - 7 and trunc(sysdate) - 1 then units else 0 end) as rep_units_lst7,
    round(sum(case when orderdate between trunc(sysdate) - 7 and trunc(sysdate) - 1 then orders else 0 end) / 7, 4) as avg_rep_orders_lst7,
    round(sum(case when orderdate between trunc(sysdate) - 7 and trunc(sysdate) - 1 then units else 0 end) / 7, 4) as avg_rep_units_lst7,
    round(sum(case when orderdate between trunc(sysdate) - 14 and trunc(sysdate) - 1 then orders else 0 end) / 14, 4) as avg_rep_orders_lst14,
    round(sum(case when orderdate between trunc(sysdate) - 14 and trunc(sysdate) - 1 then units else 0 end) / 14, 4) as avg_rep_units_lst14,
    round(sum(case when orderdate between trunc(sysdate) - 30 and trunc(sysdate) - 1 then orders else 0 end) / 30, 4) as avg_rep_orders_lst30,
    round(sum(case when orderdate between trunc(sysdate) - 30 and trunc(sysdate) - 1 then units else 0 end) / 30, 4) as avg_rep_units_lst30
  from
  (
    select 
      trunc(orh.creation_time) as orderdate,
    ...
    where
      orh.order_type = 0 and
      trunc(orh.creation_time) between trunc(sysdate) - 30 and trunc(sysdate) - 1
    group by
      trunc(orh.creation_time),
      orh.order_type,
      item.system_product_id
  ...
  select 
    prod_id,
    round(avg(stock), 4) as avg_stock_lst7,
    round(avg(case when snap_date between trunc(sysdate) - 14 and trunc(sysdate) - 1 then stock end), 4) as avg_stock_lst14,
    round(avg(case when snap_date between trunc(sysdate) - 30 and trunc(sysdate) - 1 then stock end), 4) as avg_stock_lst30,
    max(case when snap_date = trunc(sysdate) - 1 then stock else 0 end)current_stock
  from
  (
    select 
      snap_date,
      ...
      sum(units)stock
    from archive_stockonmo
    where snap_date between trunc(sysdate) - 30 and trunc(sysdate) - 1
    group by
      snap_date,
      prod_id
  )
